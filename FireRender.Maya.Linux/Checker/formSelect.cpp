...
const char g_activationPassword[] = "GPUOpen2016";
...
{
	*outmsg << KBLU << "BuildRegistrationLink begin..." << RESET << endl;
	logfile << "Building registration link...\r\n" << endl;

	/* Get the highest version selected by user */
	std::wstring mayaVersion = L"unknown";
	...
	/* Find the highest Maya version installed */
	struct stat info;
	std::string MayaInstallPathBase = "/usr/autodesk";
	if (stat(MayaInstallPathBase.c_str(), &info) != 0)
	{
		char const* path = getenv("MAYA_LOCATION");
		MayaInstallPathBase = (path == NULL) ? "/usr/autodesk" : std::string(path);
	}

	*outmsg << "MayaInstallPathBase = " + MayaInstallPathBase << endl;
	for (int i = 0; i < versions.size(); i++)
	{
		std::string MayaInstallPath = MayaInstallPathBase + "/maya" + versions[i];
		if (stat(MayaInstallPath.c_str(), &info) == 0)
			StringToWstring(mayaVersion, versions[i].c_str());
	}
	if (mayaVersion == L"unknown")
	{
		logfile << "BuildRegistrationLink: WARNING: No AutodeskMaya installation found in " << MayaInstallPathBase << ".\r\n" << endl;
		*outmsg << KRED << "WARNING MESSAGE : No AutodeskMaya installation found. Continue install anyway ?" << RESET;
...
//	*outmsg << "link must look like :" << endl;
//	*outmsg << "http://experience.amd.com/plugin-registration?registrationid=5A1E27D27D97ECF5&appname=autodeskmaya&appversion=2016&frversion=1.6.30&os=win6.1.7601&gfxcard=AMD_FirePro_W8000__FireGL_V_&driverversion=15.201.2401.0" << endl;

	g_registrationLink = L"";
	g_registrationLink += L"http://experience.amd.com/plugin-registration";
	...
	g_registrationLink += URLfriendly(mayaVersion);
	...
	*outmsg << KGRN << "Register link = " + tmp << RESET << endl;

	logfile << "Done.\r\n" << endl;
	*outmsg << KBLU << "BuildRegistrationLink end..." << RESET << endl;
}
...
bool formSelect::checkCompatibility_hardware()
{
	...
	*outmsg << KBLU << "checkCompatibility_hardware begin..." << RESET << endl;
	...
	if ( displaWarningMessage_cantGetInfo || displaWarningMessage_incompatibleOpenCL || displaWarningMessage_blacklistedDevice_names.size() >= 1 )
	{
		...
		if ( displaWarningMessage_cantGetInfo )
		{
			...
			*outmsg << KRED << "WARNING MESSAGE : "+messageWarning << RESET;
		}
		...
		if ( displaWarningMessage_blacklistedDevice_names.size() >= 1 )
		{
			string BckDevName;
	...
	logfile << "Done.\r\n" << endl;
	*outmsg << KBLU << "checkCompatibility_hardware end..." << RESET << endl;
...
bool formSelect::checkCompatibility_driver()
{
	...
	*outmsg << KBLU << "checkCompatibility_driver begin..." << RESET << endl;
...
//			*outmsg << buffer.str().c_str() << endl;
	...
	if (hardwareIsAMD)
	{
		...
		{
			driverCompatible = 1;
			*outmsg << KRED << "checkCompatibility_driver : WARNING: driver not compatible because major." << RESET << endl;
		}
		else if ( VersionMajorInt == Supported_AMD_driverMajor && VersionMinorInt < Supported_AMD_driverMinor )
		{
			driverCompatible = 1;
			*outmsg << KRED << "checkCompatibility_driver : WARNING: driver not compatible because minor." << RESET << endl;
		}
		else
		{
			driverCompatible = 2;
			*outmsg << KGRN << "checkCompatibility_driver : AMD driver compatible" << RESET << endl;
		}
	...
	else if (hardwareIsNV)
	{
		...
		if ( successParseNVdriver )
		{
			*outmsg << "checkCompatibility_driver : nvidiaPublicDriver = " + to_string(nvidiaPublicDriver) << endl;
			...
			{
				driverCompatible = 1;
				*outmsg << KRED << "checkCompatibility_driver : WARNING: NV driver too old." << RESET << endl;
			}
			else
			{
				driverCompatible = 2;
				*outmsg << KGRN << "checkCompatibility_driver : NVIDIA driver compatible" << RESET << endl;
			}
		}
		else
		{
			*outmsg << KRED << "checkCompatibility_driver : WARNING: bad NVIDIA driver format" << RESET << endl;
		}
	...
	if ( driverCompatible == 1 )
	{
		...
		messageWarning << "Continue install anyway ?";
		*outmsg << KRED << "WARNING MESSAGE : " + messageWarning.str() << RESET;
		bool result = yesno("", false);
	...
	logfile << "Done.\r\n" << endl;
	*outmsg << KBLU << "checkCompatibility_driver end..." << RESET << endl;
...
bool formSelect::checkCompatibility_Tahoe()
{
	...
	*outmsg << KBLU << "checkCompatibility_Tahoe begin..." << RESET << endl;
	...
	if (tahoe_handle)
	{
		*outmsg << KGRN << "checkCompatibility_Tahoe : libTahoe64.so can be loaded" << RESET << endl;
		dlclose(tahoe_handle);
	}
	else
	{
		*outmsg << KRED << "checkCompatibility_Tahoe - ERROR : Tahoe64.so can NOT be loaded" << RESET << endl;
	...
	*outmsg << KBLU << "checkCompatibility_Tahoe end..." << RESET << endl;
	logfile << "Done.\r\n" << endl;
...
{
	*outmsg << KBLU << "> GetSystemInfo_ begin..." << RESET << endl;
	...
	if (uname(&uts))
	{
		...
		*outmsg << KRED << "> GetSystemInfo_: FAILED - uname" << RESET << endl;
		return;
	}
	std::wstring wstr;
#if 1
	...
	if (!openclLib)
	{
		...
		{
			g_systemInfo.getInfoSuccess = false;
			*outmsg << KRED << "> GetSystemInfo_: FAILED - dlopen libOpenCL.so" << RESET << endl;
			return;
		}
	}
	*outmsg << KGRN << "> GetSystemInfo_ : " << OCL_name << " can be loaded" << RESET << endl;
	...
	{
		g_systemInfo.getInfoSuccess = false;
		*outmsg << KRED << "> GetSystemInfo_: FAILED - GetProcAddress opencl" << RESET << endl;
		return;
	}
	...
	{
		g_systemInfo.getInfoSuccess = false;
		*outmsg << KRED << "> GetSystemInfo_: FAILED - can't get OpenCL platforms" << RESET << endl;
		return;
	}
	if (platf_count == 0)
	{
		g_systemInfo.getInfoSuccess = false;
		*outmsg << KRED << "> GetSystemInfo_: FAILED - there are no available OpenCL platforms" << RESET << endl;
		return;
	}
	...
	{
		g_systemInfo.getInfoSuccess = false;
		*outmsg << KRED << "> GetSystemInfo_: FAILED - can't get OpenCL platforms 2" << RESET << endl;
	return;
	}
	...
	for (auto& plat : platforms)
	{
		...
		{
			g_systemInfo.getInfoSuccess = false;
			*outmsg << KRED << "> GetSystemInfo_: FAILED - can't get OpenCL devices" << RESET << endl;
			return;
		}
		...
		for (auto& dev : devices)
		{
			...
			{
				g_systemInfo.getInfoSuccess = false;
				*outmsg << KRED << "GetSystemInfo_: FAILED - can't get devices OpenCL version" << RESET << endl;
				return;
			}
	...
	*outmsg << KGRN << "> GetSystemInfo_: SUCCESS." << RESET << endl;
	*outmsg << KBLU << "> GetSystemInfo_ end..." << RESET << endl;
	return;
